...
public class TestNonSyncSubscriberApplication {
	...
	public static void main(String[] args) {
		...
		try {
			...
			if (applications.size() != 1) {
				System.err.println("expecting 1 application " + applicationName + ", found " + applications.size());
				return;
			}
			
			Application.Instance publisherApplication = applications.get(0);
			...
			// receiving data
			while (true) {
				int[] data = subscriber.receiveInt32();
				if (data != null) {
			...
			System.out.println("finished stream, suspended ? " + subscriber.hasEnded());
